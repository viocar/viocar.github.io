const dataSize = 104; //different from game to game. hardcoded for now.
...
window.onkeyup = function(e){
	...
	if (key == 13){ //run the code on enter
		...
		if (m_id > 271){ //check for some errors
			errorid = 1;
		...
		if (errorid == 0){ //proceed if we have no errors
			var skillTable = "https://viocar.github.io/tbl/5enemydata.tbl"; //probably should be relative but ehhhh
...
function createMonsterArray(buffer, m_id){
	...
	for (i = 0; i < 40; i++){  								//STR, VIT, AGI, LUC, TEC, WIS, attack type, acc, cut res, bash res, stab res, fire res, ice res, volt res
